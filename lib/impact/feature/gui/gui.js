...
let GuiRenderer = ig.Class.extend({
    ...
    draw: function(){
        let system = ig.system, context = system.context, scale = system.scale;
        let x = 0, y = 0;
        let steps = this.drawSteps, stepLength = steps.length;
...
ig.Gui = ig.SiteAddon.extend({
    ...
    onDeferredUpdate: function(){
        ...
        // this._updateGuiMouse(); // TODO: REWORK THIS WITH A SOLUTION THAT DOESN'T NEED THE CONTROL MODULE
        BUTTON_Z_INDEX = 0;
        this._updateRecursive(0, 0, ig.system.width, ig.system.height, c_clippingStack, true, this.guiHooks,
            0, 0, 1, true);
    },
    ...
    createEventGui: function(name, type, settings, free){
        ...
        guiElement.hook.mapGuiInfo = { name: name, type: type, settings: settings, free: free || false};
        return guiElement;
    },
    ...
    //         x = this.controlModule.getMouseX();
    //         y = this.controlModule.getMouseY();
    //     }
    //     else{
    //         x = ig.input.mouse.x;
    //         y = ig.input.mouse.y;
    ...
    //     let clicked = this.controlModule && this.controlModule.getGuiClick();
    //
    ...
    //             hook.screenCoords = { x: 0, y: 0, w: hook.size.x, h: hook.size.y, active: false, zIndex: 0};
    ...
    //         } else {
    //             isMouseOver = (coords.x <= x && coords.x + coords.w > x && coords.y <= y && coords.y + coords.h > y)
    //         }
    //
    ...
    //                 hook.gui.onMouseInteract && hook.gui.onMouseInteract(isMouseOver, clicked);
    //             }
    ...
    _updateRecursive: function(scrollX, scrollY, parentWidth, parentHeight, clippingStack, parentVisible, guiHooks,
        screenX, screenY, alpha, parentDoUpdate){
        ...
        for(let i = 0; i < guiHooks.length; ++i){
            ...
            let doUpdate = parentDoUpdate && (guiHooks !== this.guiHooks || hook.pauseGui || !ig.paused);
            ...
            let guiW = hook.size.x, guiH = hook.size.y;
            let x = scrollX, y = scrollY;
            let tScreenX = screenX, tScreenY = screenY;
            ...
            if(!hook._subState.subtreeTransition && !visible && !hook._visible && !hook.invisibleUpdate){
                continue;
            }
            ...
            let childSubtreeTransition = this._updateRecursive(hook.scroll.x, hook.scroll.y,
                guiW, guiH, clippingStack, visible,
                            hook.children, tScreenX, tScreenY, subAlpha, doUpdate);
    ...
    _drawRecursive: function(scrollX, scrollY, parentWidth, parentHeight, guiHooks){
        ...
        for(let i = 0; i < guiHooks.length; ++i){
            ...
            let x = scrollX, y = scrollY;
            switch(hook.align.x){
                case(ig.GUI_ALIGN.X_LEFT): x +=  hook.pos.x; break;
                case(ig.GUI_ALIGN.X_RIGHT): x +=  parentWidth - hook.size.x - hook.pos.x; break;
                case(ig.GUI_ALIGN.X_CENTER): x +=  Math.floor(parentWidth / 2 - hook.size.x / 2 + hook.pos.x); break;
            }
            switch(hook.align.y){
                case(ig.GUI_ALIGN.Y_TOP): y +=  hook.pos.y; break;
                case(ig.GUI_ALIGN.Y_BOTTOM): y +=  parentHeight - hook.size.y - hook.pos.y; break;
                case(ig.GUI_ALIGN.Y_CENTER): y +=  Math.floor(parentHeight / 2 - hook.size.y / 2 + hook.pos.y) ; break;
            }


            let state = hook.currentState;
            if(!hook._visible ) // Skip ig.GUI elements that are basically invisible
                continue;
            ...
            this._drawRecursive(hook.scroll.x+x, hook.scroll.y+y, hook.size.x, hook.size.y, hook.children);
  ...
  function getOffsetX(hook, parentWidth){
    switch(hook.align.x){
        case(ig.GUI_ALIGN.X_LEFT): return hook.pos.x;
        case(ig.GUI_ALIGN.X_RIGHT): return parentWidth - hook.size.x - hook.pos.x;
        case(ig.GUI_ALIGN.X_CENTER): return Math.floor(parentWidth / 2 - hook.size.x / 2 + hook.pos.x);
    }
    return 0;
}
function getOffsetY(hook, parentHeight){
    switch(hook.align.y){
        case(ig.GUI_ALIGN.Y_TOP): return hook.pos.y;
        case(ig.GUI_ALIGN.Y_BOTTOM): return parentHeight - hook.size.y - hook.pos.y;
        case(ig.GUI_ALIGN.Y_CENTER): return Math.floor(parentHeight / 2 - hook.size.y / 2 + hook.pos.y);
    }
    return 0;
}
...
ig.GuiHook = ig.Class.extend({
    ...
    doStateTransition: function(name, skipTransition, removeAfter, callback, initDelay){
        ...
        this._setStateData(transition.state, transition.time, transition.timeFunction,
            skipTransition, removeAfter, callback, initDelay);
    },
    getStateTransitionProgress: function(){
        if(!this.anim.maxTime) return 1;
        return this.anim.timer / this.anim.maxTime;
    },
    doTempStateTransition: function(stateData, time, keySpline, skipTransition, removeAfter, callback, initDelay){
        if (this.removeAfterTransition) return;
        this.removeAfterTransition = removeAfter || false;
        this.currentStateName = null;
        this._setStateData(stateData, time, keySpline, skipTransition, removeAfter, callback, initDelay);
    },
    setScale: function(x,y){
        this.currentState.scaleX = x;
        this.currentState.scaleY = y;
    },
    _setStateData: function(stateData, time, keySpline, skipTransition, removeAfter, callback, initDelay){
        if (callback)
            this.stateCallback = callback;
        ...
        if (skipTransition)
            this.currentState = ig.copy(newTargetState);
    },
    ...
    doPosTranstition: function(x,y, time, timeFunction, initDelay, preserveTime, endCallback){
        ...
        this.posTransition = {
            startX: this.pos.x, startY: this.pos.y,
            x: x, y: y, time: time, timeFunction: timeFunction || KEY_SPLINES['EASE_IN_OUT'],
            timer: (0 - (initDelay || 0)),
    ...
    doScrollTransition: function(x,y, time, timeFunction, endCallback){
        if(!time || time <= 0 || (Math.abs(x - this.scroll.x) < 0.00001 && Math.abs(y - this.scroll.y) < 0.00001)) {
            this.scroll.x = x;
            this.scroll.y = y;
        ...
        this.scrollTransition = {
            startX: this.scroll.x, startY: this.scroll.y,
            x: x, y: y, time: time, timeFunction: timeFunction || KEY_SPLINES['EASE_IN_OUT'],
            timer: 0, endCallback: endCallback || null
        }
    },
    ...
    updateState: function() {
        ...
        if (this.anim.targetState) {
            ...
            for (let key in this.anim.targetState) {
                this.currentState[key] = (1 - i) * this.anim.initState[key] + i * this.anim.targetState[key];
            }
            if (i === 1) {
...
function setGuiHookVisible(hook, visible){
    if(visible !== hook._visible){
        ...
        if(hook.gui.onVisibilityChange)
            hook.gui.onVisibilityChange(visible);
    }
}
...
ig.GuiDrawable = ig.Class.extend({
    ...
    setGfxTile: function(gfx, posX, posY, tile, tileWidth, tileHeight, flipX, flipY){
        ...
        this.setGfx(gfx, posX, posY, ( Math.floor(tile * tileWidth) % gfx.width),
                    Math.floor(tile * tileWidth / gfx.width) * tileHeight,
                    tileWidth, tileHeight, flipX, flipY);
        return this;
    },
    ...
    draw: function(x, y){
        let system = ig.system,
            ...
            scale = system.scale;
        let destX = x + this.pos.x, destY = y + this.pos.y, stateStored = false;
        let preAlpha;
        let preCompMode;
        if(this.alpha !== 1){
        ...
        if(this.gfxType === DRAWABLE_GFX_TYPE.COLOR){
            ...
            context.fillRect(system.getDrawPos(destX), system.getDrawPos(destY), this.size.x * scale, this.size.y * scale);
        }
        else if(this.gfxType === DRAWABLE_GFX_TYPE.GFX){
            this.gfxSource.draw(destX, destY, this.src.x, this.src.y, this.size.x, this.size.y, this.flip.x, this.flip.y);
        }
...
ig.GuiTransform = ig.Class.extend({
    ...
    transform: function(x, y){
        ...
        this.prePos.x = x;
        this.prePos.y = y;
        context.translate(ig.system.getDrawPos(x+this.translate.x), ig.system.getDrawPos(y+this.translate.y) );
        if(this.clip.x !== 0){
            context.beginPath();
...
ig.GuiStepPool = ig.Class.extend({
    ...
    get: function(classObj){
        if(!classObj.poolEntries)
            classObj.poolEntries = [];
...
ig.GuiElementBase = ig.Class.extend({
    ...
    doTempStateTransition: function(stateData, time, keySpline, skipTransition, removeAfter, callback, initDelay){
        this.hook.doTempStateTransition(stateData, time, keySpline, skipTransition, removeAfter, callback, initDelay);
    },
