      ...
      r_parse(text) {
        ...
        while ((index = text.indexOf('\\', parsed)) !== -1) {
          ...
          if (openBracketIndex !== -1) {
            token = text.substring(index + 1, openBracketIndex);
          ...
          if (!command) {
            ...
            let insertText;
            if (command.argument) {
              if (openBracketIndex !== index + token.length + 1 || closeBracketIndex === -1) {
                this.parsedText += text.charAt(index);
                ig.warn("Invalid Text command argument format for commant '" + token + "'");
                continue;
              }
              let argument = text.substring(openBracketIndex + 1, closeBracketIndex);
